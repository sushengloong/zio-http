  ...
  def websocketSpec = suite("Websocket Server") {
    suite("connections") {
      testM("Multiple websocket upgrades") {
        val socketApp = SocketApp.message(Socket.succeed(WebSocketFrame.text("BAR"))) ++ SocketApp.protocol(
          SocketProtocol.handshakeTimeout(100 millis),
        )
        val app       = Http.fromEffect(ZIO(Response.socket(socketApp)))
        assertM(app.websocketStatusCode(!! / "subscriptions").repeatN(1024))(equalTo(Status.SWITCHING_PROTOCOLS))
      }
    }
  }
